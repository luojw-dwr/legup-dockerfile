FROM ubuntu:14.04
MAINTAINER Jianwen LUO <luojw@shanghaitech.edu.cn>
...
# Installing GXemul
RUN mkdir gxemul_src && cd gxemul_src
COPY gxemul-0.6.0.1.tar.gz /root/gxemul_src/gxemul-0.6.0.1.tar.gz
...
RUN mkdir legup_src && cd legup_src
COPY legup-4.0.tar.gz /root/legup_src/legup-4.0.tar.gz
